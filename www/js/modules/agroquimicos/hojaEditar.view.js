...
export function initHojaEditarView() {
  ...
  form.addEventListener('change', async (e) => {
    ...
    if (e.target.classList.contains('chk-sector-editar')) {
      ...
      // Preservar los lotes que ya estaban seleccionados en el DOM
      const container = document.getElementById('editar-lotes-container');
      const lotesYaMarcados = new Set(
        [...container.querySelectorAll('.editar-chk-lote:checked')].map(cb => cb.value)
      );
      // Preservar también las hectáreas parciales ya ingresadas
      container.querySelectorAll('.lote-ha-input').forEach(input => {
        const val = parseFloat(input.value);
        if (!isNaN(val) && val > 0) {
          hectareasParciales[input.dataset.loteId] = val;
        }
      });

      const lotes = await listarLotesPorSectores(sectorIds);
      // Solo marcar como seleccionados los lotes que ya lo estaban
      renderLotesEditar(lotes, lotesYaMarcados, hectareasParciales);
      return;
    }
  ...
  form.addEventListener('submit', async e => {
    ...
    if (guardadoOk) window.showView('hojas');
  });
}
...
function resetEditarDesde(desde) {
  const orden = ['sectores', 'lotes'];
  const idx   = orden.indexOf(desde);

  if (idx <= orden.indexOf('sectores')) {
    document.getElementById('editar-sectores-container').innerHTML =
      '<strong>🗺️ Sectores</strong><p>Seleccione una empresa para ver los sectores</p>';
  }
